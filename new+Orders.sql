...
from product
where product_class_code = 2050 or product_class_code = 2051 or product_class_code = 2052
...
select pc.product_class_desc, pr.product_id, pr.product_desc, pr.product_quantity_avail,
case when pr.product_quantity_avail = 0 then 'Out of stock'

when pc.product_class_desc in ('Electronics','Computer') and pr.product_quantity_avail <= 20 then 'Low stock'
when pc.product_class_desc in ('Electronics','Computer') and pr.product_quantity_avail between 11 and 30 then 'In stock'
when pc.product_class_desc in ('Electronics','Computer') and pr.product_quantity_avail >= 31 then 'Enough stock'
 
when pc.product_class_desc in ('Stationery','Clothes') and pr.product_quantity_avail <= 20 then 'Low stock'
when pc.product_class_desc in ('Stationery','Clothes') and pr.product_quantity_avail between 21 and 80 then 'In stock'
when pc.product_class_desc in ('Stationery','Clothes') and pr.product_quantity_avail >= 81 then 'Enough stock' 

when pc.product_class_desc not in ('Electronics','Computer') and pc.product_class_desc not in ('Stationery','Clothes')
and pr.product_quantity_avail <= 15 then 'Low stock'
when pc.product_class_desc not in ('Electronics','Computer') and pc.product_class_desc not in ('Stationery','Clothes')
and pr.product_quantity_avail between 16 and 50 then 'In stock'
when pc.product_class_desc not in ('Electronics','Computer') and pc.product_class_desc not in ('Stationery','Clothes')
and pr.product_quantity_avail >= 51 then 'Enough stock' 
...
select country, count(city) as number_of_cities from (select distinct country, city from address) a
where a.country != 'usa' and a.country != 'malaysia'
group by a.country
having count(city)>1;
...
Select oc.customer_id, concat(oc.customer_fname , oc.customer_lname) as customer_fullname, ad.city, ad.pincode, 
oh.order_id, oh.order_date, pc.product_class_desc, pr.product_desc, oi.product_quantity * pr.product_price as subtotal
from 
online_customer oc 
...
where ad.pincode not like '%0%'
order by customer_fullname, order_date, subtotal;


-- 5. A stakeholder wants to find which product is ordered in highest quantity along with product_id 201.
-- In the orders that contain product id 201, find which other product is ordered in highest quantity (i.e.
-- total quantity added across the orders is highest). Display following details: product id, product
-- description, total quantity (i.e. sum(product quantity))
-- (USE SUB-QUERY) [NOTE : ORDER_ITEMS TABLE,PRODUCT TABLE] 



select o.product_id, p.product_desc, sum(o.product_quantity) from order_items o
...
oh.order_id, pr.product_desc, oi.product_quantity,  oi.product_quantity * pr.product_price as subtotal
from 
online_customer oc 
...
-- all items(len * width * height * product_quantity)) for a given orderPRODUCT_QUANTITY whose order id is 10006
-- , Assume all items of an order are packed into one single carton (box) . [NOTE : CARTON TABLE] 



select c.carton_id, c.len * c.width * c.height as carton_vol
from carton c
where c.len * c.width * c.height > (select sum(p.len * p.width * p.height * oi.product_quantity) as total_vol from product p
inner join order_items oi on p.product_id = oi.product_id where order_id ='10006')
limit 1;
...
select oc.customer_id, concat(oc.customer_fname , oc.customer_lname) as customer_fullname, oi.order_id,
...
inner join order_items oi
on oi.order_id = oh.order_id
where oh.payment_mode = 'Credit Card' or oh.payment_mode = 'Net Banking' and oh.order_status = 'shipped' 
group by oc.customer_id, customer_fullname, oi.order_id
...
inner join order_items oi
on oi.order_id = oh.order_id
where oc.customer_fname like 'A%' and oh.order_status = 'shipped' and oi.order_id > 10030
group by oi.order_id, oc.customer_id, customer_fullname;
...
-- items.
-- [NOTE:PRODUCT TABLE,ADDRESS TABLE,ONLINE_CUSTOMER
-- TABLE,ORDER_HEADER TABLE,ORDER_ITEMS TABLE,PRODUCT_CLASS TABLE] 



select product_class_desc, sum(total_quantity), sum(total_value) from 
(select a.*, product_class_desc, total_quantity * product_price as total_value from 
(select product_id, sum(product_quantity) as total_quantity from order_items where order_id in 
(select order_id from order_header where customer_id in 
(select customer_id from online_customer where address_id in 
(select address_id from address where country not like 'USA' and country not like 'India' and order_status like 'shipped')))
group by product_id) a
inner join product p on p.product_id = a.product_id
inner join product_class pc on pc.product_class_code = p.product_class_code) x
